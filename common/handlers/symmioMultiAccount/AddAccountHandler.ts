...
export class AddAccountHandler<T> extends BaseMultiAccountHandler {
	handleAccount(_event: ethereum.Event, version: MultiAccountVersion): void {
		...
		createNewAccountIfNotExists(event.params.account, event.params.user, event.address, AccountType.NORMAL, event.block, event.transaction, event.params.name, true)
	}
}