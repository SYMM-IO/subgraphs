...
def create_handler_classes(target_module, events):
	...
	for event in events:
		...
		handler_file_path = os.path.join(target_module, "handlers", handler_file_name)
...
def get_events_with_signatures(needed_events):
	...
	for event in needed_events:
		...
		if sig:
			symmio_events.append({"source": "symmio/symmio", "name": event, "signature": sig})
	...
	for event in needed_events:
		...
		if sig:
			multi_account_events.append(
				{"source": "symmioMultiAccount_0/symmioMultiAccount", "name": event, "signature": sig}
			)
