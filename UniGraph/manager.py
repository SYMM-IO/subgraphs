...
def get_event_signature(event_name, abi_file_path):
	...
	for entry in abi:
		if entry["type"] == "event" and entry["name"] == event_name:
			...
			input_types = [("indexed " if input["indexed"] else "") + input["type"] for input in entry["inputs"]]
...
def prepare_module(
	...
	if create_src_file:
		...
		generate_src_ts(target_module, multi_account_events, "_multi_account", "MultiAccount")
	if create_handler_files:
		create_handler_classes(target_module, target_config, symmio_events)
		create_handler_classes(target_module, target_config, multi_account_events, "MultiAccount")
