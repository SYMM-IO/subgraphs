...
export class SettleUpnlUnifiedHandler<T> extends BaseHandler {
	handleQuote(_event: ethereum.Event, version: Version): void {
		...
		for (let i = 0; i < event.params.settlementData.length; i++) {
			...
			quote_price_update.type = "SettleUpnlUnified"
			quote_price_update.timestamp = event.block.timestamp
			quote_price_update.save()

			setEventTimestampAndTransactionHashAndAction(quote, "SettleUpnlUnified", _event)
		}
	}
}
