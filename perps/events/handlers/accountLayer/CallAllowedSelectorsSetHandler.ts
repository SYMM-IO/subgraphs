import { CallAllowedSelectorsSet as CallAllowedSelectorsSetEntity } from "../../../../generated/schema"
import { Bytes, ethereum } from "@graphprotocol/graph-ts"
import { AccountLayerVersion } from "../../../common/BaseHandler"
import { getGlobalCounterAndInc } from "../../../common/utils"

export class CallAllowedSelectorsSetHandler<T> {
	handle(_event: ethereum.Event, version: AccountLayerVersion): void {
		// @ts-ignore
		const event = changetype<T>(_event)

		let entity = new CallAllowedSelectorsSetEntity(event.transaction.hash.toHex() + "-" + event.logIndex.toString())
